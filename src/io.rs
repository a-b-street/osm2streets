...
use geom::{Distance, FindClosest, GPSBounds, HashablePt2D, Polygon, Pt2D, Ring};
use kml::{ExtraShape, ExtraShapes};
use raw_map::{osm, RestrictionType};
...
pub fn osm_to_network(
    osm_input_path: &str,
    clip_path: Option<String>,
    timer: &mut Timer,
) -> RoadNetwork {
    let mut doc = reader::read(osm_input_path, GPSBounds::new(), timer).unwrap();
    ...
    // let mut traffic_signals = HashMap::new(),
    ...
    for (id, node) in &doc.nodes {
        ...
        //     let dir = if node.tags.is("traffic_signals:direction", "backward") { Direction::Back } else { Direction::Fwd };
